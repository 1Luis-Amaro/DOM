...
, mail.email_address                               EMAIL --08/06/2016
,(fone.phone_area_code ||' '|| fone.phone_number ) FONE  --08/06/2016                
FROM APPS.HZ_CUST_ACCOUNTS   HCA
    ,APPS.HZ_PARTIES         HP
    ,APPS.HZ_PARTY_SITES     HPS
    ,APPS.HZ_LOCATIONS       HL
    ,apps.hz_contact_points  FONE --08/06/2016
    ,apps.hz_contact_points  MAIL --08/06/2016

WHERE HP.PARTY_ID = HCA.PARTY_ID
  AND HP.PARTY_ID = HPS.PARTY_ID
  AND HL.LOCATION_ID = HPS.LOCATION_ID
  --and hp.party_name like '3%ZS%'
  --08/06/2016
  and hps.party_site_id = fone.owner_table_id(+)
  and nvl(fone.primary_flag(+),'Y') = 'Y'
  and nvl(fone.contact_point_type(+),'PHONE') = 'PHONE'
  and nvl(fone.owner_table_name(+),'HZ_PARTY_SITES') = 'HZ_PARTY_SITES'
  --
  and hps.party_site_id = mail.owner_table_id(+)
  and nvl(mail.primary_flag(+),'Y') = 'Y'
  and nvl(mail.contact_point_type(+),'EMAIL') = 'EMAIL'
  and nvl(mail.owner_table_name(+),'HZ_PARTY_SITES') = 'HZ_PARTY_SITES'