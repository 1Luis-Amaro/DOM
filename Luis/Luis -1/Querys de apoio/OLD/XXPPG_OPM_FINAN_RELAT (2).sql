SELECT /*+ parallel */
     ...
     , gxel.journal_line_type Tipo_linha
         --
     , decode(sign(gxel.entered_amount), 1, gxel.entered_amount,0,0,0)       Inf_Db
     , -decode(sign(gxel.entered_amount),-1, ABS(gxel.entered_amount),0,0,0)  Inf_Cr
     --
     , (decode(sign(gxel.entered_amount), 1, gxel.entered_amount,0,0,0))+(
      -decode(sign(gxel.entered_amount),-1, ABS(gxel.entered_amount),0,0,0))  Inf_NET
     --
     , decode(sign(gxel.accounted_amount), 1, gxel.accounted_amount,0,0,0)       BRL_Db
     , -decode(sign(gxel.accounted_amount),-1, ABS(gxel.accounted_amount),0,0,0)  BRL_Cr
     --
     , (decode(sign(gxel.accounted_amount), 1, gxel.accounted_amount,0,0,0)) +      
      (-decode(sign(gxel.accounted_amount),-1, ABS(gxel.accounted_amount),0,0,0))  BRL_NET
...
--     , translate(xae.encoded_msg, CHR(10), ' ') Msg_erro
    ...
    , (SELECT transaction_type_name
     ...
     --   
    , apps.xxppg_opm_custom_sources_pkg.get_source_document(gxeh.transaction_id,god.organization_id,gl.ledger_id,gxeh.event_class_code) Origem
    --
   ,  CASE
...
-- AND xae.ae_line_num (+)       = xdl.ae_line_num
 ...
 -- AND gxeh.transaction_date BETWEEN to_date('01/01/2015', 'dd/MM/yyyy') AND to_date('31/01/2015', 'dd/MM/yyyy')
