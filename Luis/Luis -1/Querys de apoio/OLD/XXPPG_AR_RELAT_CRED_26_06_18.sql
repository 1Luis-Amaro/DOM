    ...
    where hp.party_id = accf.party_id 
      ...
      AND accf.LAST_UPDATE_DATE IN (select MAX (accf.LAST_UPDATE_DATE)
                               ...
                               where hp.party_id = accf.party_id 
                                and accf.review_type = 'NEW_CREDIT_LIMIT')
     and accf.review_type = 'NEW_CREDIT_LIMIT'
     AND ROWNUM = 1) "OBS ULTIMA ANALISE DE CREDITO"
---------------------------------------------------------------                                                                                                                                                                                    
    FROM   
   ...
   WHERE HP.PARTY_ID = HCA.PARTY_ID
       ...
       --  AND HP.PARTY_NAME like '%LPS COMERCIO DE TINTAS LTDA%'--'%NOVO HORIZONTE COMERCIO DE TINTAS LTDA%' 
