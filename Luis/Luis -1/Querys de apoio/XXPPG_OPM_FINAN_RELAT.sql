SELECT /*+ parallel */
     ...
     , gxel.journal_line_type Tipo_linha
         --
     , round(decode(sign(gxel.entered_amount), 1, gxel.entered_amount,0,0,0),4)       Inf_Db
     , round(-decode(sign(gxel.entered_amount),-1, ABS(gxel.entered_amount),0,0,0),4)  Inf_Cr
     --
     , round((decode(sign(gxel.entered_amount), 1, gxel.entered_amount,0,0,0))+(
      -decode(sign(gxel.entered_amount),-1, ABS(gxel.entered_amount),0,0,0)),4)  Inf_NET
     --
     , round(decode(sign(gxel.accounted_amount), 1, gxel.accounted_amount,0,0,0),4)       BRL_Db
     , round(-decode(sign(gxel.accounted_amount),-1, ABS(gxel.accounted_amount),0,0,0),4)  BRL_Cr
     --
     , round((decode(sign(gxel.accounted_amount), 1, gxel.accounted_amount,0,0,0)) +      
      (-decode(sign(gxel.accounted_amount),-1, ABS(gxel.accounted_amount),0,0,0)),4)  BRL_NET
     ...
     , REPLACE(xal.description,'|','|') Descricao  
     --, xal.ae_line_num
...
--     , translate(xae.encoded_msg, CHR(10), ' ') Msg_erro
     ...
     ,REPLACE( (SELECT msi.concatenated_segments ||' | '||msi.description
    ...
    , (SELECT transaction_type_name
     ...
     --   
    , REPLACE(apps.xxppg_opm_custom_sources_pkg.get_source_document(gxeh.transaction_id,god.organization_id,gl.ledger_id,gxeh.event_class_code),'|','|') Origem
    --
   , REPLACE( CASE
         ...
         WHEN gxeh.event_class_code = 'MISC_TXN' THEN 
            'Motivo da Transacao: '||apps.xxppg_opm_custom_sources_pkg.get_txn_reason(gxeh.transaction_id,gxeh.event_class_code)
...
--  AND xah.period_name = '03-15'
--  AND gxeh.valuation_cost_type = 'STD' 
--  AND xe.transaction_date >= '01-JUN-2015'
--  AND xe.transaction_date <= '30-JUN-2015'