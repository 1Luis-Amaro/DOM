SELECT DISTINCT 
         (SELECT flv.description
           ...
           WHERE     rcta.CUSTOMER_TRX_ID = rctl.CUSTOMER_TRX_ID
                 ...
                 AND flv.LOOKUP_TYPE = 'PPG_BR_GL_BU') "DETAIL SBU",
       ...
       (SELECT Decode(Rctta.type
        ...
        (CASE WHEN Apsa.Invoice_Currency_Code != 'BRL'                        
         ...
         CREDITO.CLASSE,
         CREDITO.VALOR "VALOR LIQUIDACAO",                  
         CREDITO.DT_APLICAÇAO "DT LIQUIDACAO", 
...
(SELECT hzcp.email_address
  ...
  WHERE     NVL (hzcp.primary_flag, 'Y') = 'Y'
        ...
        -- apps.HZ_CUSTOMER_PROFILES      HCPROF,
         (SELECT (SELECT meaning
           ...
           WHERE  lookup_type = 'INV/CM'
              AND PS2.CLASS = LOOKUP_CODE) CLASSE,    
        APP.APPLY_DATE "DT_APLICAÇAO",        
        NVL (APP.AMOUNT_APPLIED_FROM, APP.AMOUNT_APPLIED) "VALOR",
        APP.APPLIED_PAYMENT_SCHEDULE_ID "PAYMENT"        
    ...
    WHERE     APP.STATUS = 'APP' 
          ...
          AND APP.PAYMENT_SCHEDULE_ID = PS2.PAYMENT_SCHEDULE_ID
          AND PS2.CUST_TRX_TYPE_ID = CTT.CUST_TRX_TYPE_ID(+)
          ...
          AND NVL (CTT.ORG_ID, PS2.ORG_ID) = PS2.ORG_ID
   UNION ALL
   SELECT 
   (SELECT meaning
            FROM   apps.ar_lookups
        ...
        APP.APPLY_DATE "DT_APLICAÇAO",
        NVL (APP.AMOUNT_APPLIED_FROM, APP.AMOUNT_APPLIED) "VALOR",
        APP.PAYMENT_SCHEDULE_ID "PAYMENT"
     FROM APPS.RA_CUSTOMER_TRX_ALL CT,
          ...
          APPS.AR_PAYMENT_SCHEDULES_ALL PS2, 
          APPS.AR_RECEIVABLE_APPLICATIONS_ALL APP  
    WHERE     APP.APPLIED_PAYMENT_SCHEDULE_ID = PS2.PAYMENT_SCHEDULE_ID
          AND PS2.CUST_TRX_TYPE_ID = CTT.CUST_TRX_TYPE_ID(+)
   ...
   SELECT NULL,
          ...
          NVL (APP.AMOUNT_APPLIED_FROM, APP.AMOUNT_APPLIED) "VALOR",
          APP.PAYMENT_SCHEDULE_ID "PAYMENT"     
     FROM APPS.AR_PAYMENT_SCHEDULES_ALL PS2,
          APPS.AR_PAYMENT_SCHEDULES_ALL PSA, 
          APPS.FND_USER FU,
          APPS.AR_RECEIVABLE_APPLICATIONS_ALL APP, 
          APPS.AR_RECEIVABLES_TRX_ALL ART 
    WHERE     APP.PAYMENT_SCHEDULE_ID = PS2.PAYMENT_SCHEDULE_ID
   ...
   UNION ALL
   SELECT (DECODE (adj.receivables_trx_id, -15, 'ASSIGNMENT', 'ADJUSTMENT')) CLASSE,
          ADJ.APPLY_DATE "DT_APLICAÇAO",
          ADJ.AMOUNT "VALOR",
          ADJ.PAYMENT_SCHEDULE_ID "PAYMENT"
   ...
   SELECT  (SELECT meaning
        ...
        APP.PAYMENT_SCHEDULE_ID "PAYMENT"
     FROM APPS.AR_PAYMENT_SCHEDULES_ALL PS2, 
          APPS.AR_PAYMENT_SCHEDULES_ALL PS_DUMMY,
          APPS.FND_USER FU, 
          APPS.AR_RECEIVABLE_APPLICATIONS_ALL APP,
          APPS.ar_receivables_trx_ALL rt,
          APPS.AR_CASH_RECEIPT_HISTORY_ALL CRH 
    WHERE     APP.PAYMENT_SCHEDULE_ID = PS2.PAYMENT_SCHEDULE_ID
          ...
          AND SIGN (APP.APPLIED_PAYMENT_SCHEDULE_ID) > 0
          AND APP.CREATED_BY = FU.USER_ID
          AND APP.ORG_ID = RT.ORG_ID) CREDITO
   WHERE     Rctla.Customer_Trx_Id = Rcta.Customer_Trx_Id
         ...
         AND CREDITO.PAYMENT (+) = Apsa.PAYMENT_SCHEDULE_ID
