...
--   and acr.receipt_number = '045981241891'
--   and hp.party_name like 'FCA%'
--   and ara.gl_date >= '02/01/2015'
...
select ara.gl_date,
      ...
      (select meaning
       ...
       gcc.segment4||'-'||gcc.segment5||'-'||gcc.segment6||'-'||
       gcc.segment7 code_combination,
       decode(rct.invoice_currency_code, 'BRL', NULL, xal.entered_dr) entered_dr,
       decode(rct.invoice_currency_code, 'BRL', NULL, xal.entered_cr) entered_cr,
  ...
  from apps.ra_customer_trx_all rct,
       (SELECT DISTINCT GL_DATE, CUSTOMER_TRX_ID, SET_OF_BOOKS_ID, EVENT_ID
          FROM apps.ar_receivable_applications_all) ARA,
...
--   and xal.accounting_class_code = 'RECEIVABLE'
   and xal.displayed_line_number > 0
--   and ara.gl_date >= '02/01/2015'
--    and rct.trx_number = '045981241891'
--   and hp.party_name like 'FCA%'
