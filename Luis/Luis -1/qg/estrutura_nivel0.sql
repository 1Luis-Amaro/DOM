...
WITH cteMenuNivel(segment1,recipe_no,inventory_item_id,qty, formula_id, recipe_id, organization_id, Nivel)
AS
(
    -- Ancora
    SELECT msi.segment1,
...
--           
           ...
           --
           msi.organization_id         IN (92,86,181,182)               --AND
           --msi.segment1 = 'AK0041L.01'
           and msi.item_type like 'ACA%'
           --and msi.segment1 = 'D800.10'
           --and msi.segment1 = 'C210-1052.32'
         --  msi.segment1 = '1501.01' and
           --msi.item_type             LIKE 'ACA%'                
           and msi.inventory_item_status_code NOT IN ('Inactive','Obsoleto') AND
...
--       
     ...
     --  msi.item_type             <> 'MISC'           AND
       ...
       --AND
       --INVENTARIO.CATEGORIA LIKE '%REF%'
       order by 1,2,3;

select * from apps.mtl_system_items_b where segment1 in ('SRC-75','HE-15-8633');

select item_type from apps.mtl_system_items_b where segment1 in ('D954.11',
'D976.11',
'D979.11')
