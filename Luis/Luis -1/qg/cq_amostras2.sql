select mp.organization_code org,
       ...
       (select max(qc.plan_name)
          from apps.qa_plan_collection_triggers_v qc
         where qc.plan_id(+)                        = qsp.plan_id and
               qc.collection_trigger_description(+) = 'Item'      and
               qc.low_value(+)                      = msi.segment1) "Plano Skip Lote",
 ...
 where mp.organization_id(+)                = gs.organization_id      and
   ...
   --    qc.low_value(+)                      = msi.segment1            and
       ...
       qsp.process_id(+)                    = qsc.process_id          and
       qsp.criteria_id(+)                   = qsc.criteria_id         and
       gs.creation_date                    >= sysdate - 365 --and sample_no = '100' -- and gs.sample_disposition <> '1P'
