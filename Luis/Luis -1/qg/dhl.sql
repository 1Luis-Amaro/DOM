SELECT TRUNC(SYSDATE) AS CREATION_DATE,
  ...
  FROM mtl_system_items_b    itm,
       ...
       mtl_onhand_quantities ohq,
       mtl_parameters        oinv,
       fnd_lookup_values_vl lv
  WHERE oinv.organization_id     = ohq.organization_id       AND
        ohq.organization_id      = itm.organization_id       AND
        ohq.inventory_item_id    = itm.inventory_item_id     AND
        ohq.subinventory_code    = lv.description            AND
        ohq.lot_number           = dlxb.nro_lote             AND
        ITM.ORGANIZATION_ID      = dlxb.org_id               AND
        ITM.SEGMENT1             = dlxb.codigo_item          AND
        lv.view_application_id    = 3 and        
        LV.LOOKUP_TYPE           = 'XPPGBR_SUBINVENTORY_DHL' AND
        LV.LOOKUP_CODE           = dlxb.subinventario        AND
        lv.enabled_flag          = 'Y'                       AND
        SYSDATE BETWEEN NVL(lv.start_date_active, SYSDATE)   AND
                        NVL(lv.end_date_active, SYSDATE)     AND        
   --     dlxb.codigo_item         = '135441.20'                 AND
...
SELECT TRUNC(SYSDATE) AS CREATION_DATE,
  ...
  FROM mtl_system_items_b   itm,
       ...
       mtl_parameters       oinv,
       fnd_lookup_values_vl lv
  WHERE oinv.organization_id     = dlxb.org_id               AND
        itm.organization_id      = dlxb.ORG_ID               AND
        itm.segment1             = dlxb.CODIGO_ITEM          AND
        lv.view_application_id   = 3 and        
        LV.LOOKUP_TYPE           = 'XPPGBR_SUBINVENTORY_DHL' AND
        ...
        SYSDATE BETWEEN NVL(lv.start_date_active, SYSDATE)   AND
                        NVL(lv.end_date_active, SYSDATE)     AND        
     --   dlxb.codigo_item       = 'D789.11'                 AND
        dlxb.ORG_ID              = 92 /*P_ORG_ID*/                  AND
        
        CODIGO_ITEM   NOT IN ( SELECT distinct(msi.SEGMENT1
                                 FROM mtl_system_items_b  msi,
                                      mtl_onhand_quantities ohq
                                 WHERE OHQ.ORGANIZATION_ID      = ITM.ORGANIZATION_ID       AND
                                       OHQ.INVENTORY_ITEM_ID    = ITM.INVENTORY_ITEM_ID     AND
                                       OHQ.SUBINVENTORY_CODE    = lv.description            AND
                                       OHQ.LOT_NUMBER           = dlxb.NRO_LOTE             AND
       ...
       AND itm.segment1 NOT IN
              (SELECT codigo_item
                ...
                WHERE     dlx.org_id = oinv.organization_id
                      ...
                      AND SYSDATE BETWEEN NVL (lv1.start_date_active, SYSDATE)
