...
FROM apps.gme_batch_header bh, apps.mtl_parameters p
WHERE batch_id in (24657)
...
FROM apps.gme_material_details md, apps.mtl_system_items_b i, apps.gme_batch_header bh
WHERE md.batch_id IN (24657)
...
t.transaction_quantity as trans_or_rsrv_qty, lt.transaction_quantity as lot_qty, 
t.transaction_uom as trans_or_rsrv_uom, t.secondary_transaction_quantity as sec_qty, 
t.secondary_uom_code, to_char(t.transaction_date, 'DD-MON-YYYY HH24:MI:SS') as trans_or_rsrv_date, 
t.LPN_ID, 
...
FROM apps.mtl_material_transactions t, apps.gme_material_details d, apps.gme_batch_header h, 
apps.mtl_transaction_lot_numbers lt, apps.mtl_lot_numbers lot, apps.mtl_system_items_b i, 
apps.mtl_transaction_types ty, apps.mtl_parameters pa
WHERE t.transaction_source_type_id = 5
AND h.batch_id in (24657)
...
r.secondary_uom_code, to_char(requirement_date, 'DD-MON-YYYY HH24:MI:SS') as trans_or_rsrv_date, 
...
FROM apps.mtl_reservations r, apps.gme_material_details d, apps.gme_batch_header h, apps.mtl_system_items_b i, 
...
secondary_quantity as sec_qty, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 
...
FROM apps.gme_pending_product_lots p, apps.gme_material_details d, apps.gme_batch_header h, 
apps.mtl_system_items_b i, apps.mtl_parameters pa
WHERE h.batch_id in (24657)
...
FROM apps.mtl_material_transactions_temp t, apps.gme_material_details d, apps.gme_batch_header h,
apps.mtl_transaction_lots_temp lt, apps.mtl_system_items_b i, apps.mtl_transaction_types ty, 
apps.mtl_parameters pa --mtl_lot_numbers lot
WHERE t.transaction_source_type_id = 5
AND h.batch_id in (24657)
...
to_char(t.creation_date, 'DD-MON-YYYY HH24:MI:SS') as creation_date, 
to_char(t.last_update_date, 'DD-MON-YYYY HH24:MI:SS') as last_update_date, NULL
FROM apps.mtl_transactions_interface t, apps.gme_material_details d, apps.gme_batch_header h,
apps.mtl_transaction_lots_interface lt, apps.mtl_system_items_b i, apps.mtl_transaction_types ty, 
apps.mtl_parameters pa --mtl_lot_numbers lot
WHERE t.transaction_source_type_id = 5
AND h.batch_id in (24657)
AND transaction_source_id = h.batch_id
...
SELECT *
FROM apps.gme_transaction_pairs
WHERE batch_id in (24657);
...
SELECT batch_id, batchstep_no, batchstep_id, step_status, gbt.oprn_id, oprn_no, oprn_vers, plan_step_qty, actual_step_qty, step_qty_um, to_char(plan_start_date, 'DD-MON-YYYY HH24:MI:SS') as plan_start_date, to_char(actual_start_date, 'DD-MON-YYYY HH24:MI:SS') as actual_start_date, to_char(plan_cmplt_date, 'DD-MON-YYYY HH24:MI:SS') as plan_cmplt_date, to_char(actual_cmplt_date, 'DD-MON-YYYY HH24:MI:SS') as actual_cmplt_date, steprelease_type, max_step_capacity, max_step_capacity_um, plan_charges, actual_charges, quality_status, routingstep_id
 FROM apps.gme_batch_steps gbt, apps.gmd_operations_b gob
 WHERE batch_id in (24657)
 ...
 FROM apps.gme_batch_step_activities
 WHERE batch_id in (24657)
...
FROM apps.gme_batch_step_resources
 WHERE batch_id in (24657)
...
SELECT doc_id as batch_id, line_id as batchstep_resource_id, poc_trans_id, resources, resource_usage, trans_qty_um, to_char(trans_date, 'DD-MON-YYYY HH24:MI:SS') as trans_date, to_char(start_date, 'DD-MON-YYYY HH24:MI:SS') as start_date, to_char(end_date, 'DD-MON-YYYY HH24:MI:SS') as end_date, completed_ind, posted_ind, overrided_protected_ind, reverse_id, delete_mark
 FROM apps.gme_resource_txns
 WHERE doc_id in (24657)
...
select r.*
  from apps.gme_batch_header b
      ,apps.gmd_recipes r
      ,apps.gmd_recipe_validity_rules vr
    where b.batch_id= 24657
...
SELECT *
FROM apps.gmf_incoming_material_layers il
WHERE (il.mmt_organization_id, il.mmt_transaction_id) IN 
    (SELECT DISTINCT t.organization_id, t.transaction_id
     ...
     WHERE t.transaction_source_id = 24657
...
SELECT *
FROM apps.gmf_outgoing_material_layers ol
...
SELECT *
FROM apps.gmf_resource_layers il
WHERE il.poc_trans_id IN 
    (SELECT t.poc_trans_id 
    FROM apps.gme_resource_txns t
    WHERE t.doc_id = 24657
    AND   t.doc_type = 'PROD');
...
SELECT *
FROM apps.gmf_batch_vib_details bvd
WHERE bvd.requirement_id IN
    (SELECT br.requirement_id
    FROM apps.gmf_batch_requirements br
    WHERE br.batch_id = 24657);
...
SELECT *
FROM apps.gmf_layer_cost_details c
WHERE
     c.layer_id IN 
    (SELECT il.layer_id
    FROM apps.gme_batch_header h, apps.mtl_material_transactions t, apps.gmf_incoming_material_layers il
    WHERE h.batch_id = 24657
...
select geh.* 
from apps.gmf_gmf_xla_extract_headers geh                
where geh.entity_code = 'PRODUCTION'
and source_document_id = 24657
and txn_source = 'PM';
...
select a.*
from apps.gl_item_dtl a
where a.itemcost_id 
in (select itemcost_id 
    from apps.gl_item_cst 
    ...
    IN (select distinct mmt.inventory_item_id, mmt.organization_id,gps.cost_type_id,gps.period_id 
        from apps.gmf_organization_definitions god,
             ...
             apps.cm_mthd_mst mthd,
             apps.mtl_material_transactions mmt
        WHERE mmt.transaction_source_type_id     = 5
              AND god.organization_id            = mmt.organization_id
              AND mmt.transaction_source_id      = 24657
...
-- SQL: FM01: Formula Header 
SELECT * 
FROM apps.FM_FORM_MST 
WHERE formula_id = 
(SELECT FORMULA_ID 
FROM apps.GME_BATCH_HEADER 
WHERE BATCH_ID = 54657 
); 


-- SQL: FM02: Formula Details 
SELECT * 
FROM apps.fm_matl_dtl 
WHERE formula_id = 
(SELECT FORMULA_ID 
FROM apps.GME_BATCH_HEADER 
WHERE BATCH_ID = 24657 
); 
