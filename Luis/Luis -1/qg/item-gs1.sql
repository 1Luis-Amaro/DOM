SELECT distinct msi.segment1 "Codigo Item",
      ...
      ''                 , --          "Número do Modelo",
       TRANSLATE(TRANSLATE(SUBSTR(MSI.DESCRIPTION,1,80),'ABCDEFGHIJKLMNOPQRSTUWVXYZabcdefghijklmnopqrstuwvxyz01234567890ÑÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕÄËÏÖÜÇñáéíóúàèìòùâêîôûãõäëïöüç–ºª²°.-!"''`#$%().:[/]{}¨+???¿;§&´*<>'
            ...
            || ' ' || trim(to_char(round((1 / nvl(cv.CONVERSION_RATE,1)),3),'9990.999')) || ' ' || msi.secondary_uom_code "Descrição do Produto (*)",
      ''                 , --          "Data lançamento mercado",
       FISCAL_CLASSIFICATION.categoria  "NCM (*)",
      ''                 , --          "CEST",
      '58000000'                   "Segmento Produto",   -- /*Cross Segment*/
      '58010000'                   "Família Produto",    -- /*Cross Segment*/
      '58010100'                   "Classe Produto",     -- /*Cross Segment*/
      '99999999'                   "Sub Classe Produto", -- /*Temporary Classification*/
      ''                 , --          "Numero de Identificação Alternativa 1",
      ''                 , --          "Agência Reguladora 1",
      ''                 , --          "Número de Identificação Alternativa 2",
      ''                 , --          "Agência Reguladora 2",
      ''                 , --          "Número de Identificação Alternativa 3",
      ''                 , --          "Agência Reguladora 3",
      ''                 , --          "Número de Identificação Alternativa 4",
      ''                 , --          "Agência Reguladora 4",
      ''                 , --          "Número de Identificação Alternativa 5",
      ''                 , --          "Agência Reguladora 5",
      'Brazil'                     "País/Mercado de Destino (*)",
      'Brazil'                     "País de Origem (*)",
      ''                 , --          "Estado",
      ''                 , --          "Altura",
      ''                 , --          "Altura - UN Medida",
      ''                 , --          "Largura",
      ...
      end "Peso Bruto"   , --          "Peso Bruto (*)",
      'kg'               , --          "Peso Bruto - UN Medida (*)",
      round(nvl(msi.unit_weight,0),3) , --          "Peso Líquido",
      'kg'               , --          "Peso Líq. - UN Medida",
      ''                 , --          "Tempo mínimo (dias) de vida útil do produto após produção",
      ''                 , --          "Compartilha Dados?",
      ''                 , --          "Observação",
      ''                 , --          "Tipo de Produto",
      ''                 , --          "Tipo de Pallet",
      ...
      ''                 , --          "Quantidade de Itens Comerciais em uma Única Camada",
      ''                 , --          "Quantidade de Itens Comerciais uma Camada Completa",
      ''                 , --          "Quantidade de Camadas Completas em Item Comercial",
      ''                 , --          "GTIN Inferior",
      ''                 , --          "Quantidade",
      ''                 , --          "Alíquota de Impostos IPI",
      'LocalWeb'         , --          "Nome URL 1 (*)",
      'FTP://186.202.69.53/'
       || trim(msi.segment1) || '.jpg' "URL", --          "URL 1 (*)",
      'Foto'             , --          "Tipo de URL 1 (*)",
      ''                 , --          "Nome URL 2 (*)",
      ''                 , --          "URL 2 (*)",
      ''                 , --          "Tipo de URL 2 (*)",
      ...
      ''                 , --          "URL 3 (*)",
      ''                 , --          "Tipo de URL 3 (*)",
      ''                 , --          "Item Comercial é um modelo",
      ''                 , --          "Unidade de Medida do Pedido",
      ''                 , --          "Múltiplo da Quantidade para Pedido",
      ''                 , --          "Quantidade Mínima para Pedido",
      ''                 , --          "Tipo da Embalagem",
      ''                 , --          "Temperatura Mínima de Armazenamento/manuseio",
      ''                 , --          "Unidade de Medida da Temperatura de Armazenamento/manuseio Mínima",
      ''                 , --          "Temperatura Máxima de Armazenamento/manuseio",
      ''                 , --          "Unidade de Medida da Temperatura Armazenamento/manuseio Máxima",
...
--       'N' "ITEM DE ENGENHARIA",
    ...
    --   MSI.INVENTORY_ITEM_STATUS_CODE,
       ...
       xref.CROSS_REFERENCE DUN14
    --   msi.inventory_item_id,
    --   xref.inventory_item_id
    , (select sum(moq.transaction_quantity) 
         from apps.mtl_onhand_quantities moq
        where moq.inventory_item_id = msi.inventory_item_id and
              moq.subinventory_code like 'D%' and
              moq.organization_id   = 92) "Saldo Sumare"
    , (SELECT max(msi1.inventory_item_status_code)
         from apps.mtl_system_items_b msi1 
        where msi1.inventory_item_id = msi.inventory_item_id and
              msi1.organization_id   = 92) status_SUM
    , (SELECT max(msi1.inventory_item_status_code)
         from apps.mtl_system_items_b msi1 
        where msi1.inventory_item_id = msi.inventory_item_id and
              msi1.organization_id   = 86) status_GVT              
--
*/
...
--
 ...
 --AND FISCAL_CLASSIFICATION.CATEGORIA LIKE '%000%'
  -- AND DUN.INVENTORY_ITEM_ID    = MSI.INVENTORY_ITEM_ID(+)
   ...
   AND msie.organization_id (+)       = wci.master_organization_id
   AND msie.inventory_item_id (+)     = wci.container_item_id
   AND wci.master_organization_id (+) = msi.organization_id
   AND cv.inventory_item_id(+)  = msi.inventory_item_id
   AND cv.to_uom_code(+)        = msi.secondary_uom_code
