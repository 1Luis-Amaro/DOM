...
select 'BRAZIL' PAIS
      ...
      ,pla.quantity  qtd_ordem
      ,cfil.quantity qtd_receb
      ,pha.currency_code
      ,xcl.produto
      ,(select full_name 
          from apps.MTL_PLANNERS mpl,
               apps.per_people_f PPF 
         where mpl.organization_id = msi.organization_id and
               mpl.planner_code    = msi.planner_code    and
               PPF.PERSON_ID       = mpl.employee_id) planejador
      ,USUARIO,
            xcl.ORGANIZATION_ID,
            xcl.HR_INICIO_RI,
            xcl.HR_TERMINO_RI,
...
where --cfi.invoice_num in ('16859')
  -- and cfi.organization_id   = 92
       msi.organization_id   = xcl.organization_id and
       msi.inventory_item_id = cfil.item_id and
   ...
   and DATA_ENTRADA_NF > sysdate - 186;
   
   
SELECT DISTINCT FULL_NAME "Planejador" FROM apps.per_people_f PPF WHERE PPF.PERSON_ID = 435 AND ROWNUM = 1 ;
