  SELECT DISTINCT
         ...
         OEL.LINE_NUMBER || '.' || OEL.SHIPMENT_NUMBER  "NUM LINHA RMA",
         OEL.ORDERED_ITEM ITEM,
         OEL.ORDERED_QUANTITY QUANTIDADE,
         OEL.UNIT_SELLING_PRICE                         "VALOR UNITARIO",
         ...
         (SELECT H.TRX_NUMBER || ' - ' || SOURCE.NAME
           ...
           WHERE     H.CUSTOMER_TRX_ID = L.CUSTOMER_TRX_ID
                 ...
                 AND H.ORG_ID          = SOURCE.ORG_ID) "NF VENDA",
         (SELECT H.TRX_NUMBER || ' - ' || SOURCE.NAME
            FROM APPS.RA_CUSTOMER_TRX_ALL H,
                 APPS.RA_CUSTOMER_TRX_LINES_ALL L,
                 APPS.RA_BATCH_SOURCES_ALL SOURCE
   ...
   WHERE     OEH.CREATED_BY         = USERS.USER_ID
         ...
         --AND OEL.FLOW_STATUS_CODE = 'CLOSED'--NOT IN ('CLOSED','CANCELLED')
         --AND oeh.HEADER_id = 262666
         --and trunc(OEH.ORDERED_DATE) between '01-JUL-2016' AND '31-JUL-2016'
        -- AND OEH.ORDER_NUMBER IN ('804317')--numero RMA
