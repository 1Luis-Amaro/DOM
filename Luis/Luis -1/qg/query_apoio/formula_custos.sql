...
select (select mic.segment2
       ...
       decode(formula_STATUS,700,'Aprovado Uso Geral',900,'Congelado') status_formula,
       decode(RECIPE_STATUS,700,'Aprovado Uso Geral',900,'Congelado') status_receita,
       decode(vr1.recipe_use,2,'Custos',0,'Producao') Uso,
       decode(validity_rule_status,700,'Aprovado Uso Geral',900,'Congelado',1000,'Obsoleto') Status_Regra,
       msi.segment1 Produto,
       msi.primary_uom_code UOM,
       vr1.preference,
       vr1.start_date, vr1.end_date,
       vr1.min_qty,
       vr1.max_qty,
       vr1.std_qty,
       vr1.PLANNED_PROCESS_LOSS
  from apps.gmd_recipes                 gr,
       apps.gmd_recipe_validity_rules   vr1,
       apps.fm_form_mst                 fm,
       apps.mtl_system_items_b          msi,
       apps.fm_rout_hdr                 frh
 where --recipe_status in (900,700) and
       --OWNER_ORGANIZATION_ID = 92 and
       ...
       msi.inventory_item_id    = vr1.inventory_item_id;
