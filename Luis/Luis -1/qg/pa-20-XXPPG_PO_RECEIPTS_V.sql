SELECT
 ...
 WHERE lookup_type = 'XXINV_INC_PA20_ORG_SUBINV'
   ...
   WHERE ffv.flex_value_set_id =ffvs.flex_value_set_id
       AND ffvs.flex_value_set_name = 'XXINV_PA20_ORGS'
) ORGNS
...
--   AND hou.name ='PPG_GLASS_US_OU'
--   AND hou.name in ('US_FBG_OU','US_FLG_OU')
   AND mp.organization_code = orgns.orgn_code
   AND orgns.orgn_code = org_subinv.orgn_code
   AND org_subinv.sicode = misv.secondary_inventory
   AND misv.organization_id = msi.organization_id
   AND misv.inventory_item_id = msi.inventory_item_id
