SELECT mp.organization_id
              ...
              , cfi.series
              , cfeo.operation_id || case when rsh.attribute1 is not null then '.' || nvl(rsh.attribute1,'') end operation_id
              , cfi.invoice_num
              , cfi.invoice_id
...
--            , NVL(mtln.transaction_quantity, mmt.transaction_quantity) transaction_quantity
          ...
          WHERE mmt.source_code              = 'RCV'
            ...
            AND mmt.transaction_type_id     IN (18,61) --ajl 29-11-2016 18-PO Receipt - 61-Int Req Intr Rcpt
           ...
           -- AND NVL(mmt.attribute15,'N')     = 'N'
            ...
            AND ( (rt.po_line_location_id IS NOT NULL AND cfil.line_location_id = rt.po_line_location_id) OR --AL 28-11-2016
                   rt.requisition_line_id IS NOT NULL)
       --
       --     AND cfil.line_location_id        = rsl.po_line_location_id
            ...
            AND assa.vendor_site_id(+)       = nvl(rsh.vendor_site_id,8796) --AJL 28-11-2016 - Sumare
            AND asu.vendor_id(+)             = assa.vendor_id
            AND msib.inventory_item_id       = mmt.inventory_item_id
            AND msib.organization_id         = mmt.organization_id
            and cfi.invoice_num = 16189 --33829 --18498
         ORDER BY cfeo.organization_id, cfeo.operation_id, cfi.invoice_id, mmt.transaction_id, mtln.lot_number;
         
         
UPDATE mtl_material_transactions
           SET attribute15 = 'N'
         WHERE transaction_id in (10622049,10622053); ,10622057);         
         
UPDATE rcv_shipment_headers rsh
              SET rsh.attribute1 = 2 --null
           WHERE rsh.shipment_header_id = 124925;       
           
select rsh.attribute1 from rcv_shipment_headers rsh
           WHERE rsh.shipment_header_id = 9076;                      