SELECT TRUNC (TO_DATE('2018-02-28','rrrr/mm/dd hh24:mi:ss')) AS CREATION_DATE,
                ...
                MAX((SELECT SUM(cst.CMPNT_COST) FROM apps.CM_CMPT_DTL cst
                       WHERE cst.delete_mark       = '0'
                         ...
                         AND cst.period_id         = (SELECT MAX(cstt.period_id)
                                                       ...
                                                       WHERE TRUNC(gps.start_date) >= TRUNC(TO_DATE('2018-02-01','rrrr/mm/dd hh24:mi:ss')) AND
                ...
                MAX((SELECT SUM(cst.CMPNT_COST) FROM apps.CM_CMPT_DTL cst
                      WHERE cst.delete_mark       = '0'
                        ...
                        AND cst.period_id         = (SELECT MAX(cstt.period_id)
                                                      ...
                                                      WHERE TRUNC(gps.start_date) >= TRUNC(TO_DATE('2018-02-01','rrrr/mm/dd hh24:mi:ss')) AND
                                                            TRUNC(gps.start_date) <= TRUNC(TO_DATE('2018-02-28','rrrr/mm/dd hh24:mi:ss'))  AND
                                                   -- WHERE TRUNC(gps.start_date)  < TRUNC(TO_DATE('2018-02-28','rrrr/mm/dd hh24:mi:ss'))   AND --'01-dez-2015'| AND
                                                            gps.cost_type_id       = cstt.cost_type_id     AND
                                                            cstt.period_id         = gps.period_id         AND
                                                            cstt.delete_mark       = '0'                   AND
                                                            cstt.organization_id   = cst.organization_id   AND
...
--                              
                   WHERE oinv.organization_id         = mmt.organization_id       AND
                         ...
                         TRUNC(mmt.transaction_date)     > TRUNC(TO_DATE('2018-02-28','rrrr/mm/dd hh24:mi:ss'))      AND
...
--                                     
           WHERE movto.organization_id(+)     = ohq.organization_id       AND
             ...
             --    itm.segment1                 = '8201144982.99'   AND
               ...
               --  oinv.resource_account        is not null                 AND                
                 ...
                 ohq.locator_id               = mil.inventory_location_id(+)                 
                 --
         ...
         SELECT TRUNC (TO_DATE('2018-02-28','rrrr/mm/dd hh24:mi:ss')) AS CREATION_DATE,
                ...
                MAX((SELECT SUM(cst.CMPNT_COST) 
                      ...
                      WHERE cst.delete_mark       = '0'
                        ...
                        AND cst.period_id         = (SELECT MAX(cstt.period_id)
                                                       FROM apps.CM_CMPT_DTL cstt, apps.gmf_period_statuses gps
                                                   -- WHERE TRUNC(gps.start_date)  < TRUNC(TO_DATE('2018-02-28','rrrr/mm/dd hh24:mi:ss'))   AND
                                                      WHERE TRUNC(gps.start_date) >= TRUNC(TO_DATE('2018-02-01','rrrr/mm/dd hh24:mi:ss')) AND
                                                            TRUNC(gps.start_date) <= TRUNC(TO_DATE('2018-02-28','rrrr/mm/dd hh24:mi:ss'))  AND
                                                            gps.cost_type_id       = cstt.cost_type_id     AND
                                                            cstt.period_id         = gps.period_id         AND
...
--                 
 ...
 --                                    
           WHERE mmt.inventory_item_id not in (select ohq.inventory_item_id 
                 ...
                 mmt.transaction_id         = mtln.transaction_id(+)    AND
                 mmt.subinventory_code   is not null                    AND
                 TRUNC(mmt.transaction_date)   > TRUNC(TO_DATE('2018-02-28','rrrr/mm/dd hh24:mi:ss')) AND --/*'2018-02-28'*/
